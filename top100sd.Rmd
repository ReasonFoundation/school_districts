...
top_sd_nces %>% write.csv("output/top_sd_nces.csv")
```
...
dict_top100 <- dictionary %>% filter(ncesID %in% top_sd_nces$ncesID)

top100_schooldistricts_3years <- sd_20_21_22 %>% 
  left_join(dictionary, by = c("state", "id")) %>% 
  left_join(nces, by = c("state", "ncesID")) %>% 
  arrange(desc(students)) %>% 
  filter(ncesID %in% dict_top100$ncesID) 

dictionary %>% filter(str_detect(name, "(?i)fairfax"))

# take from the acfrs list: 

sd_20_21_22 %>% 
  mutate(filter(str_detect(name, "(?i)Fairfax County Public Schools"))

top100_schooldistricts_3years%>% write.csv("output/top100_schooldistricts_3years.csv")
  
  
  # TODO: why does this not get joined?
  sd_20_21_22 %>% 
    filter(id == "39643") %>% 
    mutate(id = str_trim(id),
           ncesID = str_trim(ncesID)) -> test1
  
  dict_top100 %>% filter(id == "39643") %>% 
    mutate(id = str_trim(id),
          ncesID = str_trim(ncesID)) -> test2
  
test1 %>% left_join(test2)
```

SD in the top 100 NCES that are not collected in the current ACFRs database

```{r echo = FALSE}
diff_nces_acfrs_list <- setdiff(dict_top100$ncesID, top100_schooldistricts_3years$ncesID)
```

```{r}
#list of sd in top 100 NCES but NOT yet in ACFRs
sd_20_21_22 %>% 
  filter(str_detect(name, "(?i)jacksonville"))
name == "Fairfax County Public Schools" ~ "5101260",
~ "1200480"
```
