...
Problem: 
- School districts ACFRS database have names that are slightly different from the names in NCES. 

Goal: construct a dictionary that links entity name, acfrs id, nces id

# Acfrs sd that have ncesID
```{r}
# data queried June 11, 2023
...
## Dictionary 1: names matching with nces, loaded to database
```{r}
# First, filter school districts in ACFRs database that have NCES ID. These ncesIDs were added by Thuy using str_detect and matching names with nces. Ron integrated it into the database. 
...
dictionary5 <- readxl::read_xls("dictionary_5.xls") %>% # this dictionary_5.xls file is manually created in excel
...
```
Manually collect acfrs data. There's no acfrs for these entitites. Data are infered or calculated from other documents. 
Assign id to these entities as 88888
```{r}
...
```{r}
dictionary12345 <- dictionary1234 %>% left_join(nces) %>% select(-c(county_name, city)) %>% rbind(dictionary5) %>% 
  rbind(ny_city) %>% 
  rbind(delaware)


dictionary12345 %>% mutate(tot_student = sum(as.double(students), na.rm = T))
filter(str_detect(nces_original_name, "NEW YORK CITY"))
  filter(state == "NY") %>% arrange(desc(students))

saveRDS(dictionary12345, "dictionary12345.RDS")

#sum of student in dictionary12345 and DC / total students nces
(45692640 + 50971)/48026570

nces %>% filter(state %in% state.abb) 
 mutate(tot_nces = sum(students)) %>% slice(1)

# find out what nces left, not in dictionary
nces %>% filter(!ncesID %in% dictionary12345$ncesID) %>% arrange(desc(students)) %>% select(-c(county_name, city)) %>% filter(state == "CT") #%>%  #write.csv("VA_school districts_reported_in_gov.csv")

#%>% mutate(tot_nces = sum(students)) %>% slice(1)

sd2021_db %>% #filter(state == "CT") %>% arrange(name) %>% 
  filter(str_detect(name, "(?i)Arapahoe County School District No. 1"))

nces %>% filter(str_detect(nces_original_name, "(i?)Englewood School District No. 1 in the county of Arapahoe"))

dictionary12345 %>% filter(str_detect(name, "(?i)Arapahoe County School District No. 1"))
dictionary12345 %>% filter(str_detect(nces_original_name, "(?i)LADUE"))
```
